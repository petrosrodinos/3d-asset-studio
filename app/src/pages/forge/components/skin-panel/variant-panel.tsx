...
export function VariantPanel({ variant, figureId }: VariantPanelProps) {
  ...
  async function handleNameBlur() {
    ...
    setNameSaving(true);
    try {
      await apiFetch(`/api/figures/${figureId}/skins/${variant.skinId}/variants/${variant.variant}`, {
        method: "PUT",
        ...jsonInit({ name: trimmed || null }),
      });
      qc.invalidateQueries({ queryKey: ["figures"] });
    } finally {
      setNameSaving(false);
  ...
  async function handleDeleteImage(image: SkinImage) {
    await apiFetch(
      `/api/figures/${figureId}/skins/${variant.skinId}/variants/${variant.id}/images/${image.id}`,
      { method: "DELETE" },
    );
    qc.invalidateQueries({ queryKey: ["figures"] });
  }
