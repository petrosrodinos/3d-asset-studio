...
export async function runPipeline(opts: RunPipelineOpts) {
  const {
    ...
    filename,
    mimeType,
    animations = PIPELINE_CONFIG.PIPELINE_DEFAULT_ANIMATIONS,
    modelVersion,
    emitProgress,
    emitEvent,
  } = opts;
  ...
  try {
    ...
    const meshResult   = await tripo.pollTask(meshTaskId, { intervalMs: PIPELINE_CONFIG.DEFAULT_POLL_INTERVAL_MS });
    ...
    const prerigResult = await tripo.pollTask(prerigTaskId, { intervalMs: PIPELINE_CONFIG.DEFAULT_POLL_INTERVAL_MS });
    ...
    if (!prerigResult.output?.riggable) {
      ...
      emitProgress({ step: PIPELINE_CONFIG.PIPELINE_STEPS.PRERIG, status: PIPELINE_CONFIG.PIPELINE_STATUSES.FAILED, data: { error: "Model is not riggable" } });
    ...
    emitProgress({ step: PIPELINE_CONFIG.PIPELINE_STEPS.RIG, status: PIPELINE_CONFIG.PIPELINE_STATUSES.QUEUED, data: { taskId: rigTaskId } });

    await tripo.pollTask(rigTaskId, { intervalMs: PIPELINE_CONFIG.DEFAULT_POLL_INTERVAL_MS });

    await model3dSvc.finalizeModel3D(model.id, model.id, {
      pbrModelSourceUrl: pbrModelUrl,
      modelSourceUrl:    meshModelUrl,
    });
    emitProgress({ step: PIPELINE_CONFIG.PIPELINE_STEPS.RIG, status: PIPELINE_CONFIG.PIPELINE_STATUSES.SUCCESS });

    // ── 5. Animate ───────────────────────────────────────────────────────
    const animationResults: Array<{ animationKey: string; gcsGlbUrl: string; status: string }> = [];

    for (const animationKey of animations) {
      emitProgress({ step: PIPELINE_CONFIG.PIPELINE_STEPS.ANIMATE, status: PIPELINE_CONFIG.PIPELINE_STATUSES.RUNNING, data: { animationKey } });
      ...
      try {
        ...
        animationResults.push({ animationKey, gcsGlbUrl: finalAnim.gcsGlbUrl!, status: PIPELINE_CONFIG.PIPELINE_STATUSES.SUCCESS });
        emitProgress({ step: PIPELINE_CONFIG.PIPELINE_STEPS.ANIMATE, status: PIPELINE_CONFIG.PIPELINE_STATUSES.SUCCESS, data: { animationKey, gcsGlbUrl: finalAnim.gcsGlbUrl } });

      } catch (err) {
        await animationSvc.failAnimation(animRecord.id, String(err));
        animationResults.push({ animationKey, gcsGlbUrl: "", status: PIPELINE_CONFIG.PIPELINE_STATUSES.FAILED });
        emitProgress({ step: PIPELINE_CONFIG.PIPELINE_STEPS.ANIMATE, status: PIPELINE_CONFIG.PIPELINE_STATUSES.FAILED, data: { animationKey, error: String(err) } });
      }
    }
